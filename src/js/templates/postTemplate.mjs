...
export function postTemplate(
  { id, title, body, tags, media, created, updated, ...other },
  parent
) {
  ...
  let postHeader = postComponents.templatePostHeader();
  let postBody = postComponents.templatePostBody(body);
  let postFooter = postComponents.templatePostFooter();

  let post = parsedPostContainer.querySelector(".card");
